...
> Repo compatibility note: this document includes some upstream architectural hooks that were not vendored here, including `audit_data`, `cache` subcommands, and generic `new_command` examples.
...
Usage
-----
    future example command: new_command
"""
...
- Upstream-only cache management examples are not exposed as host CLI
