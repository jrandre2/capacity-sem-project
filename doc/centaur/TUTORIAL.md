...
> Repo compatibility note: all vendored CENTAUR commands in this repository run through `python src/pipeline.py centaur ...`. The upstream `audit_data` helper is not available here.
...
```bash
python src/pipeline.py centaur ingest_data --demo
```
...
# Use the stage QA outputs under data_work/centaur/quality/ instead of the upstream audit_data helper.
```
...
python src/pipeline.py centaur run_estimation -s with_controls

# Apply a sample label (custom restrictions are defined in s03_estimation.py)
python src/pipeline.py centaur run_estimation --sample restricted
```
...
python src/pipeline.py centaur validate_submission

# Validate for specific journal
python src/pipeline.py centaur validate_submission --journal jeem

# Generate report
python src/pipeline.py centaur validate_submission --journal jeem --report
```
...
Audit the pipeline data files using the generated QA and diagnostics outputs in `data_work/centaur/`.

The vendored repo does not expose the upstream `audit_data` command. Instead, inspect:
...
python src/pipeline.py centaur estimate_robustness
python src/pipeline.py centaur make_figures
python src/pipeline.py centaur validate_submission --report
...
MPLBACKEND=Agg python src/pipeline.py centaur make_figures
```
