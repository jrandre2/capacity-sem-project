...
> Repo compatibility note: this repository vendors the project analyzer, structure mapper, and migration planner. The upstream migration executor is not exposed as a runnable CLI command here.
...
- Planning migrations for manual execution
...
# Upstream executor note:
# this repo does not expose a runnable `migrate_project` CLI
# use `centaur plan_migration` and execute the plan manually
```
...
1. **Always run dry-run first**
   ```bash
   use `python src/pipeline.py centaur plan_migration --path /source --target /target`
   ```
