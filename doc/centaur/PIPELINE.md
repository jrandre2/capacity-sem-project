...
>
> **Repo compatibility note:** In this repository, vendored CENTAUR commands run through `python src/pipeline.py centaur ...`. `audit_data`, `cache`, and `migrate_project` remain upstream-only, but `review_diff`, `review_response`, and `review_ingest_docx` are available here.
...
python src/pipeline.py centaur run_stage s00_ingest
...
Review cycles are handled through `review_new`, `review_verify`, `review_diff`,
`review_response`, and `review_archive`, with manuscript-specific
`REVISION_TRACKER.md` files preserving responses. DOCX-based intake is available
through `review_ingest_docx`. Divergent drafts use manuscript variants (see
...
**Command:** `python src/pipeline.py centaur ingest_data`
...
**Command:** `python src/pipeline.py centaur link_records`
...
**Command:** `python src/pipeline.py centaur build_panel`
...
**Command:** `python src/pipeline.py centaur run_estimation [options]`
...
**Command:** `python src/pipeline.py centaur estimate_robustness`
...
**Command:** `python src/pipeline.py centaur make_figures`
...
**Command:** `python src/pipeline.py centaur validate_submission [options]`
...
- `python src/pipeline.py centaur review_status [-m manuscript]`
- `python src/pipeline.py centaur review_new [-m manuscript] [-f focus]`
- `python src/pipeline.py centaur review_new --actual [-j journal] [-r round]`
- `python src/pipeline.py centaur review_verify [-m manuscript]`
- `python src/pipeline.py centaur review_archive [-m manuscript] [--no-tag]`
...
- **Tracker-based workflow**: use `REVISION_TRACKER.md` as the review system of record
...
- `python src/pipeline.py centaur journal_parse --url https://example.com/guidelines --journal "Journal Name" --output journal.yml --save-raw`
...
python src/pipeline.py centaur draft_results --table main_results --dry-run
...
python src/pipeline.py centaur run_stage s00b_standardize
```
...
python src/pipeline.py centaur run_estimation --no-cache
python src/pipeline.py centaur estimate_robustness --no-cache
python src/pipeline.py centaur make_figures --no-cache

# Disable parallel execution
python src/pipeline.py centaur run_estimation --sequential
python src/pipeline.py centaur estimate_robustness --sequential
python src/pipeline.py centaur make_figures --sequential

# Control parallel workers (default: CPU count)
python src/pipeline.py centaur run_estimation --workers 4
python src/pipeline.py centaur estimate_robustness -w 2
```

### Cache Management

Repo note: the upstream cache subcommands were not vendored here. Use stage-level flags such as `--no-cache`, `--sequential`, and `--workers` where supported.

```bash
# Upstream-only examples omitted:
# cache stats / cache clear are not available as top-level commands in this repo.
```
...
2. **After code changes**: rerun with `--no-cache` where supported
...
Repo note: the upstream `audit_data` helper is not vendored here. Use the generated QA files under `data_work/centaur/quality/` and the stage diagnostics under `data_work/centaur/diagnostics/`.
...
python src/pipeline.py centaur link_records
python src/pipeline.py centaur build_panel
python src/pipeline.py centaur run_estimation
python src/pipeline.py centaur estimate_robustness
python src/pipeline.py centaur make_figures
cd manuscript_quarto && ./render_all.sh
```
...
python src/pipeline.py centaur run_estimation --specification baseline --engine r
