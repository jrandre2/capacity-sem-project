...
> Repo compatibility note: this repository vendors `review_new`, `review_status`, `review_verify`, `review_archive`, `review_report`, `review_diff`, `review_response`, and `review_ingest_docx`.
...
python src/pipeline.py centaur review_new --focus methods
python src/pipeline.py centaur review_new --focus economics

# Multi-manuscript support
python src/pipeline.py centaur review_new -m main -f policy
python src/pipeline.py centaur review_new -m velocity -f par_general
...
python src/pipeline.py centaur review_status -m main
```

### Step 3: Track

Document responses in:
- manuscript-specific `REVISION_TRACKER.md` files - Detailed tracking
...
| `par_general`     | Discipline | Public Administration Review framing and caution |
...
python src/pipeline.py centaur review_new --focus methods
python src/pipeline.py centaur review_new --focus economics
python src/pipeline.py centaur review_new --focus policy
python src/pipeline.py centaur review_new --focus clarity

# Multi-manuscript with focus
python src/pipeline.py centaur review_new -m main -f methods
...
### Diff And Response Helpers

`review_diff` and `review_response` are now available in this repository, so
response-material preparation can stay inside the same tracker workflow.
