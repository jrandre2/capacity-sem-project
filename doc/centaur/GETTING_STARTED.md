...
> Repo compatibility note: this repo vendors the CENTAUR analyzer, mapper, planner, stages, GUI, and manuscript tools, but not the migration executor. Use `analyze_project`, `map_project`, and `plan_migration` instead of `migrate_project`.
...
python src/pipeline.py centaur analyze_project --path /path/to/existing/project
```
...
```bash
python src/pipeline.py centaur plan_migration \
...
```bash
python src/pipeline.py centaur analyze_project \
  --path /path/to/existing/project

python src/pipeline.py centaur map_project \
  --path /path/to/existing/project
...
**Details:** See [AGENT_TOOLS.md](AGENT_TOOLS.md) for the available project-analysis and planning tools.
