...
> Repo compatibility note: this is a forward-looking design document. The
> qualitative CLI illustrated below is not vendored into this repository
> as runnable host commands.
...
```text
future CLI: qual_ingest --source transcripts/
future CLI: qual_ingest --source project.nvpx --format nvivo
future CLI: qual_ingest --source export.xlsx --format atlas
```
...
```text
future CLI: qual_code --mode deductive --codebook codebook.yaml
future CLI: qual_code --mode inductive --suggest-codes
future CLI: qual_code --mode import --source nvivo_export.xlsx
future CLI: qual_code --review
future CLI: qual_code --dry-run --provider anthropic
```
...
```text
future CLI: qual_themes --approach thematic
future CLI: qual_themes --approach framework --cases participant_id
future CLI: qual_themes --suggest --dry-run
future CLI: qual_themes --export-matrix themes_matrix.xlsx
```
...
```text
future CLI: qual_synthesize --output quotes
future CLI: qual_synthesize --output summaries --dry-run
future CLI: qual_synthesize --output profiles --cases 5
future CLI: qual_synthesize --saturation-report
```
...
```text
future CLI: qual_figures --all
future CLI: qual_figures --type network --min-cooccurrence 3
future CLI: qual_figures --type matrix --export-data
future CLI: qual_figures --type quotes --theme THEME_SYSTEMIC_BARRIERS
```
...
```text
future CLI: integrate --strategy convergent
future CLI: integrate --strategy explanatory --quant-finding "treatment_effect"
future CLI: integrate --joint-display --format xlsx
future CLI: integrate --convergence-report
```
