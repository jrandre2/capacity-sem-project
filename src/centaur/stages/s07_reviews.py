...
from ._qa_utils import generate_qa_report, QAMetrics
from ._review_models import (
    ReviewMetadata,
    parse_frontmatter,
    add_frontmatter,
)
from ..config import DOC_DIR, MANUSCRIPTS, PROJECT_ROOT, REVIEWS_DIR, DEFAULT_MANUSCRIPT
from ..utils.docx_feedback import extract_feedback
...
Format your response with numbered major and minor comments.''',
    ...
    'par_general': '''Act as a critical peer reviewer for Public Administration Review.
...
def _next_review_number(paths: dict) -> int:
    """Determine the next sequential review number for a manuscript."""
    review_num = 1
...
def _tracker_has_pending_items(tracker_file: Path) -> bool:
    """Return True if an active tracker appears to contain pending work."""
    if not tracker_file.exists():
        return False

    content = tracker_file.read_text()
    return (
        'PENDING' in content.upper()
        or bool(re.search(r'\|\s*\d+\s*\|\s*\d+\s*\|\s*\d+\s*\|\s*[1-9]\d*\s*\|', content))
    )


def _reviewer_alias_map(authors: list[str], reviewer_ids: Optional[list[str]] = None) -> dict[str, str]:
    """Map document authors/reviewers to tracker reviewer IDs."""
    if not authors:
        return {}

    if reviewer_ids and len(reviewer_ids) == len(authors):
        return {author: reviewer_ids[idx] for idx, author in enumerate(authors)}

    return {author: f"R{idx + 1}" for idx, author in enumerate(authors)}


def _normalize_inline_text(text: str) -> str:
    """Collapse multiline extracted feedback into a single line for markdown templates."""
    return ' '.join(part.strip() for part in text.splitlines() if part.strip())


def _build_docx_import_template(
    paths: dict,
    ...
    """Build a REVISION_TRACKER template from extracted DOCX feedback."""
    reviewer_lines = []
    if metadata.reviewer_ids:
        reverse_map = {rid: author for author, rid in reviewer_map.items()}
        for rid in metadata.reviewer_ids:
            author = reverse_map.get(rid)
            reviewer_lines.append(f"{rid} ({author})" if author else rid)

    header_parts = [
        f"**Document**: {paths['title']}",
        f"**Review**: #{metadata.cycle_number}",
        "**Type**: Actual (DOCX Feedback Import)",
        f"**Source File**: {docx_path}",
    ]
    if metadata.journal:
        header_parts.append(f"**Journal**: {metadata.journal}")
    if metadata.submission_round:
        header_parts.append(f"**Round**: {metadata.submission_round}")
    if metadata.decision:
        header_parts.append(f"**Decision**: {metadata.decision}")
    if reviewer_lines:
        header_parts.append(f"**Reviewers**: {', '.join(reviewer_lines)}")
    header_parts.append(f"**Imported**: {today}")
    header_info = '\n'.join(header_parts)

    major_lines = ["## Major Comments", ""]
    if summary.comments:
        reviewer_counts: dict[str, int] = {}
        for idx, comment in enumerate(summary.comments, 1):
            title = comment.title_from_text()
            reviewer_id = reviewer_map.get(comment.author)
            if reviewer_id:
                reviewer_counts[reviewer_id] = reviewer_counts.get(reviewer_id, 0) + 1
                heading = f"### {reviewer_id} Major {reviewer_counts[reviewer_id]}: {title}"
            else:
                heading = f"### Comment {idx}: {title}"

            major_lines.extend([
                heading,
                "",
                "**Status**: PENDING TRIAGE",
                f"**Reviewer**: {comment.author or 'Unknown'}",
                f"**Received**: {comment.date}",
            ])
            if comment.paragraph_text:
                major_lines.append(
                    f'**Location**: Paragraph beginning "{_normalize_inline_text(comment.paragraph_text[:120])}"'
                )

            major_lines.extend([
                "",
                "**Reviewer's Concern**:",
                f"> {_normalize_inline_text(comment.text) or '[No comment text extracted]'}",
                "",
        ...
        major_lines.extend([
            "_No Word comments were extracted from the source document._",
            "",
        ])

    minor_lines = ["## Minor Comments", ""]
    if summary.changes:
        for idx, change in enumerate(summary.changes, 1):
            if change.change_type == 'insertion':
                concern = f"Track change insertion proposed by {change.author or 'Unknown'} on {change.date}: `{_normalize_inline_text(change.new_text)}`"
            elif change.change_type == 'deletion':
                concern = f"Track change deletion proposed by {change.author or 'Unknown'} on {change.date}: `{_normalize_inline_text(change.original_text)}`"
            else:
                concern = f"Track change from {change.author or 'Unknown'} on {change.date}"

            minor_lines.extend([
                f"### Minor {idx}: Track change - {change.change_type}",
                "",
                "**Status**: PENDING TRIAGE",
                f"**Concern**: {concern}",
                "",
        ...
        minor_lines.extend([
            "_No track changes were extracted from the source document._",
            "",
        ])
    ...
    if summary.total_items == 0:
        note_lines = [
            "## Import Note",
            "",
            "No embedded Word comments or tracked changes were found in this source document.",
            "This tracker exists so the manuscript can still be attached to the unified review workflow",
            "and re-imported later if a commented DOCX version is received.",
            "",
            "---",
            "",
        ]

    template = f'''# Revision Tracker: Imported DOCX Feedback
...
|----------|-------|-----------|--------------|---------|
| Major Comments | {summary.comment_count} | 0 | 0 | {summary.comment_count} |
| Minor Comments | {summary.change_count} | 0 | 0 | {summary.change_count} |

{chr(10).join(note_lines)}{chr(10).join(major_lines)}
---
...
- [ ] All imported comments triaged (VALID/INVALID/BEYOND SCOPE/ALREADY ADDRESSED)
- [ ] All imported track changes accepted or rejected
- [ ] Responses documented for each imported item
- [ ] Modified files listed
- [ ] Manuscript re-rendered and verified
- [ ] If needed, response letter generated and checked

---

*DOCX feedback imported: {today}*
*Last updated: {today}*
'''
    return template
...
def status(manuscript: str = None):
    ...
    if frontmatter:
        ...
        if frontmatter.get('source_file'):
            print(f"Source file: {frontmatter.get('source_file')}")
...
def new_cycle(
    ...
    review_num = _next_review_number(paths)

    # Check if there's an active review
    tracker_file = paths['tracker_file']
    if _tracker_has_pending_items(tracker_file):
...
def ingest_docx_review(
    ...
    """
    Import Word comments/track changes into the unified review tracker format.

    Parameters
    ----------
    manuscript : str, optional
        Manuscript key. Defaults to DEFAULT_MANUSCRIPT.
    docx_path : str or Path, optional
        Path to the source DOCX. If omitted, uses manuscript config `source_docx`.
    journal : str, optional
        Journal name to record in metadata.
    submission_round : str, optional
        Submission round, e.g. initial, R&R1.
    decision : str, optional
        Editorial decision label.
    reviewer_ids : list[str], optional
        Explicit reviewer identifiers. If omitted and authors are present in the
        DOCX comments, IDs are inferred as R1, R2, ...
    dry_run : bool
        When True, print tracker content without writing files.
    force : bool
        Overwrite an existing active tracker.
    """
    paths = get_manuscript_paths(manuscript)
    manuscript_key = manuscript or DEFAULT_MANUSCRIPT
    manuscript_config = MANUSCRIPTS[manuscript_key]
    tracker_file = paths['tracker_file']

    if docx_path is None:
        docx_path = manuscript_config.get('source_docx')
        if docx_path is None:
            print(f"ERROR: No default DOCX source configured for manuscript '{manuscript_key}'.")
            print("Provide one with --input /path/to/file.docx")
            return None
    ...
    if tracker_file.exists() and not force:
        print(f"ERROR: Active tracker already exists: {tracker_file}")
        print("Archive it first or rerun with --force to overwrite.")
        return None

    summary = extract_feedback(docx_path)
    authors = sorted(summary.authors)
    reviewer_map = _reviewer_alias_map(authors, reviewer_ids)
    effective_reviewers = reviewer_ids or list(reviewer_map.values())
    review_num = _next_review_number(paths)
    today = datetime.now().strftime('%Y-%m-%d')
    ...
    metadata = ReviewMetadata(
        ...
        submission_round=submission_round,
        decision=decision,
        reviewer_ids=effective_reviewers,
        source_file=docx_path,
        start_commit=get_current_commit(),
        tracker_file=tracker_file,
    )

    template = _build_docx_import_template(
        paths=paths,
        metadata=metadata,
        docx_path=docx_path,
        summary=summary,
    ...
    if dry_run:
        print(final_content)
        print("\n" + "=" * 50)
        print(f"Dry run summary: comments={summary.comment_count}, track_changes={summary.change_count}")
        return tracker_file
    ...
    print(f"Imported DOCX review source: {docx_path}")
    print(f"Created tracker: {tracker_file}")
    print(f"Comments extracted: {summary.comment_count}")
    print(f"Track changes extracted: {summary.change_count}")
    if authors:
        print(f"Reviewer mapping: {', '.join(f'{rid}={author}' for author, rid in reviewer_map.items())}")
    else:
        print("Reviewer mapping: none inferred from DOCX metadata")

    print("\nNext steps:")
    print("1. Triage each imported item")
    print("2. Update responses and modified files")
    print(f"3. Run: python src/pipeline.py review_verify --manuscript {manuscript_key}")
...
def generate_response_letter(
    ...
    pending = sum(
        1 for c in comments
        if (
            'ACTION NEEDED' in c['status'].upper()
            or 'PENDING' in c['status'].upper()
        ) and (
            not c['response'] or c['response'] == '[Describe response]' or 'TO BE' in c['response'].upper()
        )
    )
